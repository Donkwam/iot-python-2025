...
# 객체지향 다시
class Car:
    ## __new__ 사용빈도 낮음, __init__ 많이사용
    ## Car() 호출하면 아래의 메서드가 실행
    ## company, name, plateNumber 모를때는 None(먼지 모름)
    def __init__(self, company=None, name=None, plateNumber=None):
        self.__company = company    # 멤버변수이름 앞에 __ 쓰면 외부 접근 불가
        self.__name = name
    ...
    def setName(self, newName='글쎄요'): # 이름을 모를때 (글쎄요)로 대체
        self.__name = newName
...
# myCar.__plateNumber = 2018  # 클래스 외부에서 잘못되 데이터를 넣어도 문제가 발생X
# print(myCar)
